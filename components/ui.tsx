...
export function CardHeader({ children, className = '' }: { children: React.ReactNode, className?: string }) {
  return <div className={`px-6 py-5 border-b border-slate-700/30 ${className}`}>{children}</div>;
}

export function CardTitle({ children, className = '' }: { children: React.ReactNode, className?: string }) {
  return <h3 className={`font-bold text-white tracking-wide ${className}`}>{children}</h3>;
}
...
export function Button({ 
  ...
  const variants = {
    ...
    outline: "border border-slate-700/50 bg-slate-800/30 hover:bg-slate-800/60 text-slate-200",
